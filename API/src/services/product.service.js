...
async function getAllProducts(reqQuery) {
    ...
    if(color) {
        ...
        colorRegex = colorSet.size ? new RegExp([...colorSet].join('|'), i) : null;
    ...
    if(sizes) {
        ...
        query.where('sizes.name').in([...sizeSet]); //query query
    }
    ...
    if(stock) {
        if (stock == 'in_stock') {
            query = query.query.where('quantity').gt(0);
        }
        else if (stock == 'out_of_stock') {
            query = query.query.where('quantity').lt(1);
        }
    }
